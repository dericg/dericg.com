...
class GPLVault_Admin {
	...
	public function actions_links( $links ) {
		...
		$plugin_path   = 'admin.php?page=' . self::SLUG_PLUGINS;
		$plugin_links  = '<a href="' . self_admin_url( $plugin_path ) . '">' . __( 'GV Plugins', 'gplvault' ) . '</a>';
	...
	public function add_help_tabs( $screen ) {
		...
		$screen->add_help_tab(
			array(
				...
				sprintf( '<p>%s</p>', sprintf( __( 'On version 4.0.0, we have added a custom plugins upgrade system at <a href="%1$s">GPLVault > Plugins</a> and dropped the support for WP Native upgrade system. From version 4.1.0, we have bring back WP Native upgrade system along with custom upgrade facility as a fallback way.', 'gplvault' ), self_admin_url( 'admin.php?page=' . self::SLUG_PLUGINS ) ) ) .
				sprintf( '<span><strong>%1$s</strong>: %2$s</span>', __( 'Notes', 'gplvault' ), __( 'The search option on custom Plugins Update page is not functional yet.', 'gplvault' ) ) .
				sprintf(
					'<p><strong>%1$s</strong>: %2$s</p>',
					__( 'Custom Update System', 'gplvault' ),
					__( 'Our custom plugins update system is an Ajax based plugins upgrade tool which upgrades plugins listed on GPLVault.com main site and have automatic API capability. On above linked page you can update plugins using our Custom Update System.', 'gplvault' )
				) .
				sprintf(
					'<p><strong>%1$s</strong>: %2$s</p>',
		...
		$screen->add_help_tab(
			array(
				'id'      => 'gplvault_tab_themes',
				'title'   => __( 'Themes', 'gplvault' ),
				'content' =>
					'<h2>' . __( 'GPLVault Themes Upgrade', 'gplvault' ) . '</h2>' .
					'<p>' .
						sprintf( __( 'GPLVault provided themes are upgrade via WP native upgrade system. On <strong>Version 4.0.0 to 4.0.3</strong> theme upgrade system is missing. But this feature is added again in the <strong>Version 4.1.0</strong>. So you are able to upgrade themes from <a href="%1$s" target="_blank">Appearance > Themes</a> in WP Admin Dashboard.', 'gplvault' ), self_admin_url( 'themes.php' ) ) .
					'</p>',
			)
		);
